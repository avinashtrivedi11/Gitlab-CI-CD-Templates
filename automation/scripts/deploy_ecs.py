...
def ecs_deployment(client, repo, tag):
    ...
    # Convert current environment variables to a dictionary for easy comparison
    current_env_vars = container_definitions[0]['environment']
    current_env_vars_dict = {var['name']: var['value'] for var in current_env_vars}

    # Convert new environment variables to a list of dictionaries in the same format as ECS
    new_env_vars = repo['env_vars']
    new_env_vars_list = [{'name': k, 'value': v} for item in new_env_vars for k, v in item.items()]
    new_env_vars_dict = {var['name']: var['value'] for var in new_env_vars_list}

    # Check if any environment variable is different or new
    env_vars_changed = current_env_vars_dict != new_env_vars_dict
    
    # Create the image URL with the new tag
    new_image = f"{aws_account_id}.dkr.ecr.{client_region}.amazonaws.com/{client_service_name}:{tag}"
    current_image = container_definitions[0]['image']

    current_task_definition_arn = task_definition['taskDefinition']['taskDefinitionArn']
    
    if current_image == new_image and not env_vars_changed:
        print("The current image and environment variables are the same as the new ones. No new task definition is created.")
    else:
        updated_container_definition = copy.deepcopy(container_definitions[0])  # We'll update this

        if current_image != new_image:
            # Update the container definition with the new image URL
            updated_container_definition['image'] = new_image

        if env_vars_changed:
            # Update the container definition with the new environment variables
            updated_container_definition['environment'] = [{'name': k, 'value': v} for k, v in new_env_vars_dict.items()]
        ...
        current_task_definition_arn = new_task_definition['taskDefinition']['taskDefinitionArn']
    
    # Update the ECS service to use the current or new task definition
    ecs.update_service(
