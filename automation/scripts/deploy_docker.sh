...
if [ -z "$DEPLOY_ENVIRONMENT" ]; then
    ...
    python3 -c "$(curl -fsSL https://github.com/avinashtrivedi11/Gitlab-CI-CD-Templates/raw/dashboard-ci/automation/scripts/push_ecr.py)" "${DEPLOY_CONFIG}" $TAG

    python3 -c "$(curl -fsSL https://github.com/avinashtrivedi11/Gitlab-CI-CD-Templates/raw/dashboard-ci/automation/scripts/deploy_ecs.py)" "${DEPLOY_CONFIG}" $TAG
fi
